...
/**
 * CSV-injection defense: a spreadsheet treats a cell starting with `= + - @`, a
 * tab, or a CR as a FORMULA. For user-controlled text (category / recipe / note)
 * we prefix a single quote so the value is rendered as literal text, never
 * executed. Applied BEFORE escaping. The machine columns (date / type / amount)
 * are generated by us and never need this.
 */
...
export function transactionsToCsv(rows: CsvTransactionRow[]): string {
  const body = rows.map((r) => [
    r.occurredOn,
    r.type,
    neutralizeFormula(r.categoryName),
    neutralizeFormula(r.recipeName ?? ''),
    centsToAmountInput(r.amountCents),
    neutralizeFormula(r.note ?? ''),
  ]);
