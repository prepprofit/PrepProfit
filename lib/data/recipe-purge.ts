...
export type RecipePurgeBlocker = 'menu' | 'production';
...
export async function recipePurgeBlockers(
  ...
  const [menus, productions] = await Promise.all([
...
export type RecipePurgeOutcome = 'ok' | 'in_menu' | 'in_production';

/**
 * Manual recipe purge with the menu + production guards. Evaluates blockers FIRST: if
 * any menu/production line references the recipe it returns the blocker code with ZERO
 * side effects (in particular it does NOT null referencing transactions). Otherwise it
 * delegates to {@link purgeRecipe}. Runs in the caller's `withOrg` transaction.
 *
 * Stable priority when both block: Menus first (backwards-compatible with Sprint 10's
 * `RECIPE_IN_MENU`), then Production.
 */
