...
- [~] Pricing 4-tier + reverse trial (`docs/pricing-4-tier-plan.md`) - add the **Solo** (€19) tier between Free and Pro (4 tiers total; `break_even` drops to Solo+), rebalance the AI-quota ladder, and introduce a 14-day **reverse trial** (every new org = Business access, then Free). Slices 1-6 DONE on `main` (entitlements core, Clerk catalogue + `solo` plan with paid trials removed, reverse-trial webhook/claim/gating, 4-column pricing UI + copy, enforcement tests + docs, and the daily trial-ending reminder cron). No DB migration (trial lives in Clerk org metadata + session claim; the reminder cron reads that metadata directly and sends once via a single-calendar-day window + Resend idempotency). Seat enforcement: the subscription webhook now syncs each org's Clerk `max_allowed_memberships` to its paid tier (`clerkSeatLimit`), so Pro=5/Business=∞ are real; free/trial orgs stay unmanaged (documented). **Clerk DEV instance fully configured via `clerk config patch`** (solo plan, paid trials off, `session.claims.org_trial_ends_at`, ai_extraction feature removed from plans). **OWNER-PENDING:** provision the Clerk PROD instance (`clerk deploy`, interactive) then apply the same config patches on `--instance prod` (€19 EUR); `/api/cron/trial-reminder` already deploys from `vercel.json` with `CRON_SECRET` set. Live mapping: CLAUDE.md "Subscription plans" + `lib/entitlements.ts`.
